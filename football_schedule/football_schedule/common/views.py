...
#     data.append([schedule_date.team_generation])
#     data.append([f"треньор {schedule_date.coach}"])
#     data.append([schedule_date.month])
#     for week in weeks:
#         week_dates = [
#             f"{week.start_date.strftime('%d.%m')}-{(week.start_date + timedelta(days=6)).strftime('%d.%m')}"
#         ]
...
#     # Save DataFrame to Excel in memory
#     with pd.ExcelWriter(output, engine='openpyxl') as writer:
#         df.to_excel(writer, sheet_name="Schedule", index=False, startrow=0)  # Start data at row 3 (to leave space for title and weekly titles)
#
#     # Load the workbook from the in-memory buffer
#     output.seek(0)
#     wb = load_workbook(output)
...
#     ws.merge_cells(start_row=10, start_column=1, end_row=10, end_column=7)
#     ws.merge_cells(start_row=14, start_column=1, end_row=14, end_column=7)
#     ws.merge_cells(start_row=18, start_column=1, end_row=18, end_column=7)
#     ws.merge_cells(start_row=22, start_column=1, end_row=22, end_column=7)
#
#     # Define border style
#     thin_border = Border(
#         left=Side(style='thin'),
